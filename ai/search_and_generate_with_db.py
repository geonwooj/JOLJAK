...
class PatentDBManager:
    ...
    def fetch_pending_input(self):
        """
        '사용자입력DB' 테이블에서 아직 처리되지 않은(status='pending') 
        가장 최근의 아이디어를 하나 가져옵니다.
        """
        query = "SELECT id, idea_text FROM 사용자입력DB WHERE status = 'pending' ORDER BY created_at ASC LIMIT 1"
        self.cur.execute(query)
        return self.cur.fetchone() # (input_id, idea_text)

    def update_input_status(self, input_id, status='completed'):
        """입력 데이터의 처리 상태 업데이트"""
        self.cur.execute("UPDATE 사용자입력DB SET status = ? WHERE id = ?", (status, input_id))
        self.conn.commit()
...
def run_pipeline():
    ...
    # 1. DB에서 대기 중인 사용자 입력 가져오기
    pending_data = db.fetch_pending_input()
    if not pending_data:
        print("[INFO] 처리할 대기 중인 사용자 입력이 없습니다.")
        return

    input_id, user_idea = pending_data
    print(f"\n[START] ID {input_id}번 아이디어 처리 시작: {user_idea[:20]}...")

    try:
        # 2. 데이터 로드 및 분야 예측
        # 기존 search_index 로직 활용
        ...
        # p_ids를 search_with_strategy의 인자로 사용하도록 수정 필요 (기존 코드는 files 사용)
        results = search_with_strategy(w_query, p_ids, p_types, w_vectors)

        # 4. 선행기술 Context 추출 (DB JOIN 활용)
        prior_arts = []
        for r in results[:5]:
        ...
        # 6. 결과 기록 및 '상태 업데이트' (중요!)
        db.log_result(user_idea, top_field, claim)
        db.update_input_status(input_id, status='completed')
        print(f"✅ ID {input_id} 처리 완료 및 DB 상태 'completed' 변경 완료")

    except Exception as e:
        print(f"❌ ID {input_id} 처리 중 오류 발생: {e}")
        db.update_input_status(input_id, status='failed') # 실패 로그를 남기면 디버깅이 쉽습니다.

if __name__ == "__main__":# 이제 인자를 받지 않고 바로 DB 체크를 시작합니다.
    run_pipeline()